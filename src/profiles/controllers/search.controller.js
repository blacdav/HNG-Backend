...
const parseGender = (text) => {
    ...
    // Handle "male and female" or "female and male"
    if (/\b(?:male|female)\s+and\s+(?:male|female)\b/.test(normalized)) {
        return "both"; // Special value to indicate both genders
    }
...
export const SearchProfile = async (req, res) => {
    ...
    const pageIndex = Math.max(1, parseInt(page, 10) || 1);
    ...
    // Parse keywords - only for name and optional country search
    const keywords = normalizedSearch ? cleanSearchKeywords(normalizedSearch) : null;
    if (keywords) {
        where[Op.or] = [
            { name: { [Op.like]: `%${keywords}%` } }
        ];
        // Add country to OR only if it wasn't already parsed
        if (!countryValue) {
    ...
    // Check for incomplete query patterns (words that require parameters but don't have them)
    ...
    // If incomplete patterns exist, check if corresponding filters were actually parsed
    if (hasIncompletePatterns) {
        // Check if age filters were parsed when age-related incomplete patterns exist
        ...
        // If age incomplete patterns exist but no age filter was parsed, reject
        ...
        // If sort incomplete patterns exist but no sort was parsed, reject
    ...
    // Check if we were able to parse any meaningful filters
    ...
    try {
        ...
        return res.status(500).json({
            status: "error",
            message: "Internal server error while processing search"
        });
    }
};
